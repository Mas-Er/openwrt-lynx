...
amlogic_path="${make_path}/include"
...
depends_repo="https://github.com/lynxnexy/openwrt/tree/main/include"
...
find_openwrt() {
    ...
    # Find custom DISTRIB_SOURCECODE, such as [ openwrt or rooter ]
    [[ -f "${temp_dir}/${source_release_file}" ]] && {
        ...
        [[ -n "${source_codename}" && "${source_codename:0:1}" != "_" ]] && source_codename="${source_codename}"
...
make_image() {
    ...
    build_image_file="${out_path}/${source_codename}_${board}_k${kernel}_$(date +"%Y.%m.%d").img"
